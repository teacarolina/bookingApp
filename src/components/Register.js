...
<div className="flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
  <div className="max-w-md w-full space-y-8">
    ...
    <form className="mt-8 space-y-6" onSubmit={onHandleSubmit} method="POST">
      ...
      <div>
          <input id="username" name="username" value={registerValues.username} onChange={onHandleChange} type="text" required className="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Användarnamn"/>
        </div>
        <div>
          <input id="email" name="email" value={registerValues.email} onChange={onHandleChange} type="text" required className="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Emailadress"/>
        </div>
        <div>
          <input id="password" name="password" value={registerValues.password} onChange={onHandleChange} type="password" required className="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Lösenord"/>
        </div>
      </div>
      <div>
        ...
        <Link to="/login" className="text-gray-900 hover:bg-pink-50 hover:text-gray-900 block px-3 py-2 rounded-md text-base font-medium">Har du redan ett konto? Logga in här 😍</Link>
      </div>
    </form>
  </div>
</div>
